...
import { MdKeyboardArrowDown, MdLogoDev } from "react-icons/md";
...
const NavItem = ({ item }) => {
    ...
    return (
        <li className="relative group">
            {item.type === "link" ? (
                <>
                    <Link to={item.href} className="px-4 py-2 hover:text-primary hover:text-lg">
                        {item.name}
                    </Link>
                ...
                <div className="flex flex-col items-center">
                    <button className="px-4 py-2 flex gap-x-1 items-center hover:text-primary hover:text-lg" onClick={togglerDropdown}>
                        <span>{item.name}</span>
                        <MdKeyboardArrowDown />
                    </button>
                        ...
                        <ul className="bg-dark-soft lg:bg-transparent text-center flex flex-col shadow-lg rounded-lg overflow-hidden">
                            {item.items.map((page, index) => (
                                <Link
                                    key={index}
                                    to={page.href}
                                    className="hover:bg-dark-hard hover:text-white px-4 py-2 text-white lg:bg-primary lg:text-white z-10"
                                >
                                    {page.title}
                                </Link>
                            ))}
